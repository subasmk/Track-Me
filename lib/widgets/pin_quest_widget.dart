...
/// Pins a goal streak widget for [goalId] (or the first goal).
Future<void> pinGoalWidgetWithFeedback(BuildContext context, String? goalId, String? title) async {
  final messenger = ScaffoldMessenger.of(context);
  context.read<GoalService>().notifyExternalChange();
  final result = await HomeWidgetService.pinGoalWidget(goalId);
  final name = title == null ? 'goal streak' : '"$title"';
  final message = switch (result) {
    PinWidgetResult.requested => 'Confirm on the next screen to add the $name widget.',
    PinWidgetResult.unsupported =>
      "Your launcher can't add widgets from inside the app. Long-press the home screen, "
          'tap Widgets, and drag "TrackMe" out.',
    PinWidgetResult.failed => "Couldn't request the widget. Try adding it from the home screen.",
  };
